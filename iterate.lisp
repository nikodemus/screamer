;;; -*- Mode: LISP; Package: (ITERATE :USE CL :COLON-MODE :EXTERNAL); Base: 10; Syntax: Ansi-common-lisp -*-
...
(defpackage :iterate
 ...
 (:export :iterate :iter :display-iterate-clauses :declare-variables
	  ...
	  #-cmu :finish
          ...
          :never :thereis :finding :collect :collecting :with :while
          :until :adjoining :nconcing :appending :nunioning :unioning
...
(export '(iterate iter display-iterate-clauses declare-variables
          defmacro-clause defclause-sequence dsetq initially
          after-each finally if-first-time finally-protected #-akcl finish
          leave next-iteration next terminate repeat for as
          generate generating in sum summing multiply multiplying
          maximize minimize maximizing minimizing counting always
          never thereis finding collect collecting with while
...
#+(or poplog akcl) (use-package :lisp)

#-(or lucid poplog akcl) (declaim (declaration declare-variables))
...
(defun make-initial-value (value value-supplied? type)
  ...
  (cond
   ...
   ((subtypep type 'character)
    (coerce #-(or poplog akcl) #\null #+(or poplog akcl) (int-char 0) type))
   (t 
    (clause-warning 
...
(defun free-vars (form bound-vars)
  ...
  (cond
   ...
   ((symbolp (car form))
    (cond
     ((or (#-(or ansi-90 allegro cmu) special-form-p
	     #+(or ansi-90 allegro cmu) special-operator-p (car form)) 
...
(defun free-vars-fspec (fspec bound-vars)
  ...
  (if (or (symbolp fspec)
          (eq (car fspec) 'setf)
          #+symbolics (eq (car fspec) 'lisp:setf))
      nil
